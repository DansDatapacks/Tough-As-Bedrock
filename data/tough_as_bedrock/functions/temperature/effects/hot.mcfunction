...
execute if score #difficulty dan_tab.data matches 3 as @s[scores={dan_tab.health=..3},tag=!dan_tab.gamemodeAdmin,tag=!dan_tab.dead] run function tough_as_bedrock:temperature/effects/hot_death